...
if (!$blog->failed && ($blog->ownerID == $sessionObj->user->ID || $blog->checkMemberPermission($sessionObj->user->ID, 'read_asks'))) {
    if ($globalInbox) {
        $messages = $blog->getAllOutboxes();
    } else {
        $messages = $blog->getOutbox();
    }
    foreach ($messages as $message) {
        $message->outboxRender();
    }
} else {
    echo ':(';
}
		        ...
		        <div id="outboxList">
                    ...
                    <?php 
                            ...
                            foreach ($blogList as $blog) {?>
                                <li class="nav-item">
                                <a class="nav-link inbox-link" href="https://<?php echo $_ENV['SITE_URL']; ?>/outbox/<?php echo $blog->blogName;?>"><?php echo $blog->blogName; ?></a>
                              </li>
                              <?php 
                            }
                            ?>
...
<script src="https://<?php echo $_ENV['SITE_URL']; ?>/js/delete-outbox.js"></script>
