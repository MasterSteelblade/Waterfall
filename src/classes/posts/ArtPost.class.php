...
class ArtPost extends Post {
    ...
    public function createNew($postText, $postTitle, $postTags, $onBlog, $additions, $type, $imageIDs) {
        ...
        $values = array('art', $this->content, $this->postTitle, $this->database->php_to_postgres($this->getTagIDs($this->tags)), $this->onBlog, $this->postStatus, $this->timestamp->format("Y-m-d H:i:s.u"), $this->database->php_to_postgres($this->inlineImages), $pollID, $this->database->php_to_postgres($imageIDs));
