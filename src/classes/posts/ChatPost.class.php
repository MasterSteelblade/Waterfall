...
/**
 * Chat Post extension for Posts. Contains functionality specific to making Chat Posts. 
 */
    public function createNew($postText, $postTitle, $postTags, $onBlog, $additions, $type) {
        ...
        $values = array('chat', $this->content, $this->postTitle, $this->database->php_to_postgres($this->getTagIDs($this->tags)), $this->onBlog, $this->postStatus, $this->timestamp->format("Y-m-d H:i:s.u"), $this->database->php_to_postgres($this->inlineImages), $pollID);
        $result = $this->database->db_insert("INSERT INTO posts (post_type, post_content, post_title, tags, on_blog, post_status, timestamp, inline_images, poll_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)", $values);
        if ($result) {
